...
주식주문(현금) API for [국내주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `(매도) TTTC0011U (매수) TTTC0012U` |
| Paper Trading | `(매도) VTTC0011U (매수) VTTC0012U` |
...
| `tr_id` | `(매도) TTTC0011U (매수) TTTC0012U` | Conditional | TR_ID for real trading |
| `tr_id` | `(매도) VTTC0011U (매수) VTTC0012U` | Conditional | TR_ID for paper trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `(매도) TTTC0011U (매수) TTTC0012U`, Paper: `(매도) VTTC0011U (매수) VTTC0012U`) | Yes |
...
# Paper Trading
TR_ID = "(매도) VTTC0011U (매수) VTTC0012U"

def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 주식주문(현금).
