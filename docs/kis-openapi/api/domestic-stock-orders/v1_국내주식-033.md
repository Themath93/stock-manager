...
퇴직연금 미체결내역 API for [국내주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `TTTC2201R(기존 KRX만 가능), TTTC2210R (KRX,NXT/SOR)` |
...
| `tr_id` | `TTTC2201R(기존 KRX만 가능), TTTC2210R (KRX,NXT/SOR)` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `TTTC2201R(기존 KRX만 가능), TTTC2210R (KRX,NXT/SOR)`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 퇴직연금 미체결내역.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-stock/v1/trading/pension/inquire-daily-ccld"
