...
주식일별주문체결조회 API for [국내주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `(3개월이내) TTTC0081R (3개월이전) CTSC9215R` |
| Paper Trading | `(3개월이내) VTTC0081R (3개월이전) VTSC9215R` |
...
| `tr_id` | `(3개월이내) TTTC0081R (3개월이전) CTSC9215R` | Conditional | TR_ID for real trading |
| `tr_id` | `(3개월이내) VTTC0081R (3개월이전) VTSC9215R` | Conditional | TR_ID for paper trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `(3개월이내) TTTC0081R (3개월이전) CTSC9215R`, Paper: `(3개월이내) VTTC0081R (3개월이전) VTSC9215R`) | Yes |
...
# Paper Trading
TR_ID = "(3개월이내) VTTC0081R (3개월이전) VTSC9215R"

def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 주식일별주문체결조회.
