...
해외선물 체결추이(일간) API for [해외선물옵션] 기본시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `HHDFC55020100` |
...
```
GET /uapi/overseas-futureoption/v1/quotations/daily-ccnl
```
...
| `tr_id` | `HHDFC55020100` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `HHDFC55020100`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외선물 체결추이(일간).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/overseas-futureoption/v1/quotations/daily-ccnl"
