...
해외주식 현재가 호가 API for [해외주식] 기본시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `HHDFS76200100` |
...
```
GET /uapi/overseas-price/v1/quotations/inquire-asking-price
```
...
| `tr_id` | `HHDFS76200100` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `HHDFS76200100`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 현재가 호가.
