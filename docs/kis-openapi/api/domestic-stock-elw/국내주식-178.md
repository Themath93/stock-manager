...
ELW 변동성 추이(일별) API for [국내주식] ELW 시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPEW02840200` |
...
```
GET /uapi/elw/v1/quotations/volatility-trend-daily
```
...
| `tr_id` | `FHPEW02840200` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPEW02840200`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to ELW 변동성 추이(일별).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/elw/v1/quotations/volatility-trend-daily"
