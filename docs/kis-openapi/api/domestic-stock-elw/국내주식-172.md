...
ELW 투자지표추이(체결) API for [국내주식] ELW 시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPEW02740100` |
...
```
GET /uapi/elw/v1/quotations/indicator-trend-ccnl
```
...
| `tr_id` | `FHPEW02740100` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPEW02740100`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to ELW 투자지표추이(체결).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/elw/v1/quotations/indicator-trend-ccnl"
