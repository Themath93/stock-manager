...
ELW 투자지표추이(분별) API for [국내주식] ELW 시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPEW02740300` |
...
```
GET /uapi/elw/v1/quotations/indicator-trend-minute
```
...
| `tr_id` | `FHPEW02740300` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPEW02740300`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to ELW 투자지표추이(분별).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/elw/v1/quotations/indicator-trend-minute"
