...
선물옵션 일중예상체결추이 API for [국내선물옵션] 기본시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPIF05110100` |
...
```
GET /uapi/domestic-futureoption/v1/quotations/exp-price-trend
```
...
| `tr_id` | `FHPIF05110100` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPIF05110100`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 선물옵션 일중예상체결추이.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-futureoption/v1/quotations/exp-price-trend"
