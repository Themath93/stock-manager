...
(야간)선물옵션 주문체결 내역조회 API for [국내선물옵션] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `(구) JTCE5005R (신) STTN5201R` |
...
```
GET /uapi/domestic-futureoption/v1/trading/inquire-ngt-ccnl
```
...
| `tr_id` | `(구) JTCE5005R (신) STTN5201R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `(구) JTCE5005R (신) STTN5201R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to (야간)선물옵션 주문체결 내역조회.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-futureoption/v1/trading/inquire-ngt-ccnl"
