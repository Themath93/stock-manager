...
선물옵션 주문 API for [국내선물옵션] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `(주간 매수/매도) TTTO1101U (야간 매수/매도) (구) JTCE1001U (신) STTN1101U` |
| Paper Trading | `(주간 매수/매도) VTTO1101U (야간은 모의투자 미제공)` |


## Endpoint

```
POST /uapi/domestic-futureoption/v1/trading/order
```
...
| `tr_id` | `(주간 매수/매도) TTTO1101U (야간 매수/매도) (구) JTCE1001U (신) STTN1101U` | Conditional | TR_ID for real trading |
| `tr_id` | `(주간 매수/매도) VTTO1101U (야간은 모의투자 미제공)` | Conditional | TR_ID for paper trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `(주간 매수/매도) TTTO1101U (야간 매수/매도) (구) JTCE1001U (신) STTN1101U`, Paper: `(주간 매수/매도) VTTO1101U (야간은 모의투자 미제공)`) | Yes |
...
# Paper Trading
TR_ID = "(주간 매수/매도) VTTO1101U (야간은 모의투자 미제공)"

def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 선물옵션 주문.
