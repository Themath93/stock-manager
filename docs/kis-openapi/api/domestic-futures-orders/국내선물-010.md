...
(야간)선물옵션 잔고현황 API for [국내선물옵션] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `(구) JTCE6001R (신) CTFN6118R` |
...
```
GET /uapi/domestic-futureoption/v1/trading/inquire-ngt-balance
```
...
| `tr_id` | `(구) JTCE6001R (신) CTFN6118R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `(구) JTCE6001R (신) CTFN6118R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to (야간)선물옵션 잔고현황.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-futureoption/v1/trading/inquire-ngt-balance"
