...
(야간)선물옵션 증거금 상세 API for [국내선물옵션] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `(구) JTCE6003R (신) CTFN7107R` |
...
```
GET /uapi/domestic-futureoption/v1/trading/ngt-margin-detail
```
...
| `tr_id` | `(구) JTCE6003R (신) CTFN7107R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `(구) JTCE6003R (신) CTFN7107R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to (야간)선물옵션 증거금 상세.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-futureoption/v1/trading/ngt-margin-detail"
