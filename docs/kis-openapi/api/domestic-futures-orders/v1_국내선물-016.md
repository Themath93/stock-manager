...
선물옵션 기준일체결내역 API for [국내선물옵션] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `CTFO5139R` |
...
```
GET /uapi/domestic-futureoption/v1/trading/inquire-ccnl-bstime
```
...
| `tr_id` | `CTFO5139R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `CTFO5139R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 선물옵션 기준일체결내역.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-futureoption/v1/trading/inquire-ccnl-bstime"
