...
장내채권 매수가능조회 API for [장내채권] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `TTTC8910R` |
...
```
GET /uapi/domestic-bond/v1/trading/inquire-psbl-order
```
...
| `tr_id` | `TTTC8910R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `TTTC8910R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 장내채권 매수가능조회.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-bond/v1/trading/inquire-psbl-order"
