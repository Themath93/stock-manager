...
장내채권 주문체결내역 API for [장내채권] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `CTSC8013R` |
...
```
GET /uapi/domestic-bond/v1/trading/inquire-daily-ccld
```
...
| `tr_id` | `CTSC8013R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `CTSC8013R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 장내채권 주문체결내역.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-bond/v1/trading/inquire-daily-ccld"
