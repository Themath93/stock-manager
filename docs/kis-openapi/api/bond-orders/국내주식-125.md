...
장내채권 정정취소주문 API for [장내채권] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `TTTC0953U` |
...
```
POST /uapi/domestic-bond/v1/trading/order-rvsecncl
```
...
| `tr_id` | `TTTC0953U` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `TTTC0953U`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 장내채권 정정취소주문.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-bond/v1/trading/order-rvsecncl"
