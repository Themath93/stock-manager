...
NAV 비교추이(일) API for [국내주식] 기본시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPST02440200` |
...
| `tr_id` | `FHPST02440200` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPST02440200`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to NAV 비교추이(일).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/etfetn/v1/quotations/nav-comparison-daily-trend"
