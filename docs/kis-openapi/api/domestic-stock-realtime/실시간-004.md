...
국내주식 실시간호가 (KRX) API for [국내주식] 실시간시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `H0STASP0` |
| Paper Trading | `H0STASP0` |


## Endpoint

```
POST /tryitout/H0STASP0
```
...
| `tr_id` | `H0STASP0` | Conditional | TR_ID for real trading |
| `tr_id` | `H0STASP0` | Conditional | TR_ID for paper trading |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 실시간호가 (KRX).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/tryitout/H0STASP0"
