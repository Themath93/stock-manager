...
국내주식 실시간체결가 (KRX) API for [국내주식] 실시간시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `H0STCNT0` |
| Paper Trading | `H0STCNT0` |
...
| `tr_id` | `H0STCNT0` | Conditional | TR_ID for real trading |
| `tr_id` | `H0STCNT0` | Conditional | TR_ID for paper trading |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 실시간체결가 (KRX).
