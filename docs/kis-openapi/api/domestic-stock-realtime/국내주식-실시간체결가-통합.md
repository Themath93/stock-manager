...
국내주식 실시간체결가 (통합) API for [국내주식] 실시간시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `H0UNCNT0` |
...
| `tr_id` | `H0UNCNT0` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `H0UNCNT0`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 실시간체결가 (통합).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/tryitout/H0UNCNT0"
