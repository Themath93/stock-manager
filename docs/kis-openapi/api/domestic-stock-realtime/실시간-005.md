...
국내주식 실시간체결통보 API for [국내주식] 실시간시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `H0STCNI0` |
...
| `tr_id` | `H0STCNI0` | Conditional | TR_ID for real trading |
| `tr_id` | `H0STCNI9` | Conditional | TR_ID for paper trading |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 실시간체결통보.
