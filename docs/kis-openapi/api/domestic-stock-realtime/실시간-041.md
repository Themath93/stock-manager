...
국내주식 실시간예상체결 (KRX) API for [국내주식] 실시간시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `H0STANC0` |
...
| `tr_id` | `H0STANC0` | Conditional | TR_ID for real trading |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 실시간예상체결 (KRX).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/tryitout/H0STANC0"
