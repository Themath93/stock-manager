...
국내주식 실시간회원사 (NXT) API for [국내주식] 실시간시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `H0NXMBC0` |
...
| `tr_id` | `H0NXMBC0` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `H0NXMBC0`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 실시간회원사 (NXT).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/tryitout/H0NXMBC0"
