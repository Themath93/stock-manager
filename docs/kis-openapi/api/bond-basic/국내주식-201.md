...
장내채권현재가(체결) API for [장내채권] 기본시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHKBJ773403C0` |
...
```
GET /uapi/domestic-bond/v1/quotations/inquire-ccnl
```
...
| `tr_id` | `FHKBJ773403C0` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHKBJ773403C0`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 장내채권현재가(체결).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-bond/v1/quotations/inquire-ccnl"
