...
채권지수 실시간체결가 API for [장내채권] 실시간시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `H0BICNT0` |
...
| `tr_id` | `H0BICNT0` | Conditional | TR_ID for real trading |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 채권지수 실시간체결가.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/tryitout/H0BICNT0"
