...
종합 시황/공시(제목) API for [국내주식] 업종/기타.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHKST01011800` |
...
| `tr_id` | `FHKST01011800` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHKST01011800`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 종합 시황/공시(제목).
