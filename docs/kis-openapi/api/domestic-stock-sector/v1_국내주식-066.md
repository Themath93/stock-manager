...
국내업종 구분별전체시세 API for [국내주식] 업종/기타.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPUP02140000` |
...
| `tr_id` | `FHPUP02140000` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPUP02140000`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내업종 구분별전체시세.
