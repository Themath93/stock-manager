...
국내주식 예상체결 전체지수 API for [국내주식] 업종/기타.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHKUP11750000` |
...
```
GET /uapi/domestic-stock/v1/quotations/exp-total-index
```
...
| `tr_id` | `FHKUP11750000` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHKUP11750000`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 예상체결 전체지수.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-stock/v1/quotations/exp-total-index"
