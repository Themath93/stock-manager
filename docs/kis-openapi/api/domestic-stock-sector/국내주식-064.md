...
국내업종 시간별지수(초) API for [국내주식] 업종/기타.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPUP02110100` |
...
```
GET /uapi/domestic-stock/v1/quotations/inquire-index-tickprice
```
...
| `tr_id` | `FHPUP02110100` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPUP02110100`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내업종 시간별지수(초).
