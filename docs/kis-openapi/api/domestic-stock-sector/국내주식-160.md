...
국내선물 영업일조회 API for [국내주식] 업종/기타.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `HHMCM000002C0` |
...
```
GET /uapi/domestic-stock/v1/quotations/market-time
```
...
| `tr_id` | `HHMCM000002C0` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `HHMCM000002C0`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내선물 영업일조회.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-stock/v1/quotations/market-time"
