...
국내주식 대량체결건수 상위 API for [국내주식] 순위분석.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHKST190900C0` |
...
```
GET /uapi/domestic-stock/v1/ranking/bulk-trans-num
```
...
| `tr_id` | `FHKST190900C0` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHKST190900C0`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 대량체결건수 상위.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-stock/v1/ranking/bulk-trans-num"
