...
국내주식 수익자산지표 순위 API for [국내주식] 순위분석.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPST01730000` |
...
| `tr_id` | `FHPST01730000` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPST01730000`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 수익자산지표 순위.
