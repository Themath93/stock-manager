...
국내주식 체결강도 상위 API for [국내주식] 순위분석.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPST01680000` |
...
```
GET /uapi/domestic-stock/v1/ranking/volume-power
```
...
| `tr_id` | `FHPST01680000` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPST01680000`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 체결강도 상위.
