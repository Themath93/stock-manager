...
프로그램매매 종합현황(일별) API for [국내주식] 시세분석.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPPG04600001` |
...
```
GET /uapi/domestic-stock/v1/quotations/comp-program-trade-daily
```
...
| `tr_id` | `FHPPG04600001` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPPG04600001`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 프로그램매매 종합현황(일별).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-stock/v1/quotations/comp-program-trade-daily"
