...
종목별 프로그램매매추이(체결) API for [국내주식] 시세분석.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPPG04650101` |
...
```
GET /uapi/domestic-stock/v1/quotations/program-trade-by-stock
```
...
| `tr_id` | `FHPPG04650101` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPPG04650101`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 종목별 프로그램매매추이(체결).
