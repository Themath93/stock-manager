...
시장별 투자자매매동향(시세) API for [국내주식] 시세분석.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPTJ04030000` |
...
| `tr_id` | `FHPTJ04030000` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPTJ04030000`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 시장별 투자자매매동향(시세).
