...
종목별 외인기관 추정가집계 API for [국내주식] 시세분석.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `HHPTJ04160200` |
...
| `tr_id` | `HHPTJ04160200` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `HHPTJ04160200`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 종목별 외인기관 추정가집계.
