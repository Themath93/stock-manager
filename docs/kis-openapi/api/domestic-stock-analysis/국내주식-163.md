...
회원사 실시간 매매동향(틱) API for [국내주식] 시세분석.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPST04320000` |
...
```
GET /uapi/domestic-stock/v1/quotations/frgnmem-trade-trend
```
...
| `tr_id` | `FHPST04320000` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPST04320000`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 회원사 실시간 매매동향(틱).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-stock/v1/quotations/frgnmem-trade-trend"
