...
국내주식 체결금액별 매매비중 API for [국내주식] 시세분석.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHKST111900C0` |
...
```
GET /uapi/domestic-stock/v1/quotations/tradprt-byamt
```
...
| `tr_id` | `FHKST111900C0` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHKST111900C0`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 국내주식 체결금액별 매매비중.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-stock/v1/quotations/tradprt-byamt"
