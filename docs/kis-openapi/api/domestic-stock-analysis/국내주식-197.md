...
주식현재가 회원사 종목매매동향 API for [국내주식] 시세분석.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `FHPST04540000` |
...
```
GET /uapi/domestic-stock/v1/quotations/inquire-member-daily
```
...
| `tr_id` | `FHPST04540000` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `FHPST04540000`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 주식현재가 회원사 종목매매동향.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-member-daily"
