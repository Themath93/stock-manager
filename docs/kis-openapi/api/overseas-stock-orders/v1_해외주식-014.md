...
해외주식 매수가능금액조회 API for [해외주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `TTTS3007R` |
| Paper Trading | `VTTS3007R` |
...
| `tr_id` | `TTTS3007R` | Conditional | TR_ID for real trading |
| `tr_id` | `VTTS3007R` | Conditional | TR_ID for paper trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `TTTS3007R`, Paper: `VTTS3007R`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 매수가능금액조회.
