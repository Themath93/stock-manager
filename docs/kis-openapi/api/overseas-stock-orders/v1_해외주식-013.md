...
해외주식 예약주문조회 API for [해외주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `(미국) TTTT3039R (일본/중국/홍콩/베트남) TTTS3014R` |
...
```
GET /uapi/overseas-stock/v1/trading/order-resv-list
```
...
| `tr_id` | `(미국) TTTT3039R (일본/중국/홍콩/베트남) TTTS3014R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `(미국) TTTT3039R (일본/중국/홍콩/베트남) TTTS3014R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 예약주문조회.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/overseas-stock/v1/trading/order-resv-list"
