...
해외주식 체결기준현재잔고 API for [해외주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `CTRP6504R` |
| Paper Trading | `VTRP6504R` |
...
| `tr_id` | `CTRP6504R` | Conditional | TR_ID for real trading |
| `tr_id` | `VTRP6504R` | Conditional | TR_ID for paper trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `CTRP6504R`, Paper: `VTRP6504R`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 체결기준현재잔고.
