...
해외증거금 통화별조회 API for [해외주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `TTTC2101R` |
...
```
GET /uapi/overseas-stock/v1/trading/foreign-margin
```
...
| `tr_id` | `TTTC2101R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `TTTC2101R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외증거금 통화별조회.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/overseas-stock/v1/trading/foreign-margin"
