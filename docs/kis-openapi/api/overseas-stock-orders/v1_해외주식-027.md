...
해외주식 미국주간정정취소 API for [해외주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `TTTS6038U` |
...
```
POST /uapi/overseas-stock/v1/trading/daytime-order-rvsecncl
```
...
| `tr_id` | `TTTS6038U` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `TTTS6038U`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 미국주간정정취소.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/overseas-stock/v1/trading/daytime-order-rvsecncl"
