...
해외주식 미국주간주문 API for [해외주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `(주간매수) TTTS6036U (주간매도) TTTS6037U` |
...
| `tr_id` | `(주간매수) TTTS6036U (주간매도) TTTS6037U` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `(주간매수) TTTS6036U (주간매도) TTTS6037U`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 미국주간주문.
