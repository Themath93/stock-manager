...
해외주식 결제기준잔고 API for [해외주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `CTRP6010R` |
...
```
GET /uapi/overseas-stock/v1/trading/inquire-paymt-stdr-balance
```
...
| `tr_id` | `CTRP6010R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `CTRP6010R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 결제기준잔고.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/overseas-stock/v1/trading/inquire-paymt-stdr-balance"
