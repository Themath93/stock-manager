...
해외주식 지정가체결내역조회 API for [해외주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `TTTS6059R` |
...
```
GET /uapi/overseas-stock/v1/trading/inquire-algo-ccnl
```
...
| `tr_id` | `TTTS6059R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `TTTS6059R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 지정가체결내역조회.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/overseas-stock/v1/trading/inquire-algo-ccnl"
