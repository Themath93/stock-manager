...
해외주식 예약주문접수취소 API for [해외주식] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `(미국 예약주문 취소접수) TTTT3017U (아시아국가 미제공)` |
| Paper Trading | `(미국 예약주문 취소접수) VTTT3017U (아시아국가 미제공)` |


## Endpoint

```
POST /uapi/overseas-stock/v1/trading/order-resv-ccnl
```
...
| `tr_id` | `(미국 예약주문 취소접수) TTTT3017U (아시아국가 미제공)` | Conditional | TR_ID for real trading |
| `tr_id` | `(미국 예약주문 취소접수) VTTT3017U (아시아국가 미제공)` | Conditional | TR_ID for paper trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `(미국 예약주문 취소접수) TTTT3017U (아시아국가 미제공)`, Paper: `(미국 예약주문 취소접수) VTTT3017U (아시아국가 미제공)`) | Yes |
...
# Paper Trading
TR_ID = "(미국 예약주문 취소접수) VTTT3017U (아시아국가 미제공)"

def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 예약주문접수취소.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/overseas-stock/v1/trading/order-resv-ccnl"
