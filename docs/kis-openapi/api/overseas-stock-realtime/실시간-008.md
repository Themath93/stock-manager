...
해외주식 지연호가(아시아) API for [해외주식] 실시간시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `HDFSASP1` |
...
| `tr_id` | `HDFSASP1` | Conditional | TR_ID for real trading |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 지연호가(아시아).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/tryitout/HDFSASP1"
