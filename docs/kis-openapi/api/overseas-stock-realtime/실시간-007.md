...
해외주식 실시간지연체결가 API for [해외주식] 실시간시세.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `HDFSCNT0` |
...
| `tr_id` | `HDFSCNT0` | Conditional | TR_ID for real trading |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외주식 실시간지연체결가.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/tryitout/HDFSCNT0"
