...
예탁원정보(배당일정) API for [국내주식] 종목정보.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `HHKDB669102C0` |
...
| `tr_id` | `HHKDB669102C0` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `HHKDB669102C0`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 예탁원정보(배당일정).
