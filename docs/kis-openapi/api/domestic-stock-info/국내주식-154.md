...
예탁원정보(주주총회일정) API for [국내주식] 종목정보.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `HHKDB669111C0` |
...
```
GET /uapi/domestic-stock/v1/ksdinfo/sharehld-meet
```
...
| `tr_id` | `HHKDB669111C0` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `HHKDB669111C0`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 예탁원정보(주주총회일정).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-stock/v1/ksdinfo/sharehld-meet"
