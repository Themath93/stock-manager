...
예탁원정보(의무예치일정) API for [국내주식] 종목정보.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `HHKDB669110C0` |
...
```
GET /uapi/domestic-stock/v1/ksdinfo/mand-deposit
```
...
| `tr_id` | `HHKDB669110C0` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `HHKDB669110C0`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 예탁원정보(의무예치일정).

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/domestic-stock/v1/ksdinfo/mand-deposit"
