...
해외선물옵션 정정취소주문 API for [해외선물옵션] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `(정정) OTFM3002U (취소) OTFM3003U` |
...
| `tr_id` | `(정정) OTFM3002U (취소) OTFM3003U` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `(정정) OTFM3002U (취소) OTFM3003U`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외선물옵션 정정취소주문.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/overseas-futureoption/v1/trading/order-rvsecncl"
