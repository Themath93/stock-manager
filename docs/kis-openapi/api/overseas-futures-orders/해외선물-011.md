...
해외선물옵션 일별 체결내역 API for [해외선물옵션] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `OTFM3122R` |
...
```
GET /uapi/overseas-futureoption/v1/trading/inquire-daily-ccld
```
...
| `tr_id` | `OTFM3122R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `OTFM3122R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외선물옵션 일별 체결내역.
