...
해외선물옵션 미결제내역조회(잔고) API for [해외선물옵션] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `OTFM1412R` |
...
| `tr_id` | `OTFM1412R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `OTFM1412R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외선물옵션 미결제내역조회(잔고).
