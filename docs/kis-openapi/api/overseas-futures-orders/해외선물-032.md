...
해외선물옵션 증거금상세 API for [해외선물옵션] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `OTFM3115R` |
...
```
GET /uapi/overseas-futureoption/v1/trading/margin-detail
```
...
| `tr_id` | `OTFM3115R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `OTFM3115R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외선물옵션 증거금상세.

    Returns:
        Dict[str, Any]: API response data
    """
    url = f"{BASE_URL}/uapi/overseas-futureoption/v1/trading/margin-detail"
