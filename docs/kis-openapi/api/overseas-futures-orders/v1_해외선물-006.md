...
해외선물옵션 주문가능조회 API for [해외선물옵션] 주문/계좌.

## TR_ID Information

| Environment | TR_ID |
|-------------|-------|
| Real Trading | `OTFM3304R` |
...
```
GET /uapi/overseas-futureoption/v1/trading/inquire-psamount
```
...
| `tr_id` | `OTFM3304R` | Conditional | TR_ID for real trading |
...
|-----------|------|-------------|----------|
| `tr_id` | string | TR_ID (Real: `OTFM3304R`, Paper: `모의투자 미지원`) | Yes |
...
def make_api_request() -> Dict[str, Any]:
    """
    Make API request to 해외선물옵션 주문가능조회.
